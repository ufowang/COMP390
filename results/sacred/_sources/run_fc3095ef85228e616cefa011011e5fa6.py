...
from components.episode_buffer import PriorityReplayBuffer
...
def run_sequential(args, logger):
    ...
    if args.priority_replay_buffer:
        buffer = PriorityReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
                                      args.prb_alpha, args.prb_beta, args.prb_eps, args.t_max,
    ...
    # Learner
    learner = le_REGISTRY[args.learner](mac, buffer.scheme, logger, args, buffer)
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            for _ in range(args.training_iters):
                if args.priority_replay_buffer:
                    batch = buffer.sample(args.batch_size, runner.t_env)
                else:
                ...
                episode_sample.is_weights = batch.is_weights
                episode_sample.indices = batch.indices
