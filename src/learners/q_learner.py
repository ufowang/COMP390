...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.priority_replay_buffer:
            is_weights = batch.is_weights.unsqueeze(1).unsqueeze(2).expand_as(td_error)
            loss = ((masked_td_error ** 2) * is_weights).sum() / mask.sum()
        else:
        ...
        if self.args.priority_replay_buffer:
            priorities = (td_error.abs().mean(dim=(1, 2))).detach().cpu().numpy()
